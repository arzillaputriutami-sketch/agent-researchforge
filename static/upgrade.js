/**
 * ResearchForge Aggregator — dApp Upgrade Wiring
...
(function() {
  ...
  async function apiFetch(path, opts = {}) {
    try {
      ...
      console.error('[ResearchForge Aggregator] API error:', e);
  ...
  window.dappExportJSON = function(data, filename) {
    ...
    a.href = url;
    a.download = filename || '03-researchforge-export.json';
    a.click();
  ...
  window.dappExportCSV = function(rows, filename) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = filename || '03-researchforge-export.csv';
    a.click();
    URL.revokeObjectURL(url);
